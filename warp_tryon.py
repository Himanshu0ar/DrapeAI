...
with mp_pose.Pose(static_image_mode=True) as pose:
    ...
    # Step 1: Garment ko rectangle jaisa full-size canvas pe paste karo
    garment_pil = Image.open(garment_output_path).convert("RGBA").resize((garment_width, garment_height))
    canvas = Image.new("RGBA", (image_width, image_height), (0, 0, 0, 0))
    canvas.paste(garment_pil, (paste_x, paste_y), garment_pil)
    canvas_np = np.array(canvas)

    # Step 2: Source (rectangle) aur Target (trapezoid) points define karo
    source_pts = np.array([[
        [paste_x, paste_y],
    ...
    # Step 3: Warped garment ko person image ke upar composite karo
    warped_pil = Image.fromarray(warped)
    person_pil = Image.open(person_path).convert("RGBA")
    person_pil.paste(warped_pil, (0, 0), warped_pil)

    final_output_path = "output_images/final_tryon_warped.png"
    person_pil.save(final_output_path)
    print("Warped try-on saved to:", final_output_path)
